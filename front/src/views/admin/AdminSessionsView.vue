    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-8">
      ...
      <form @submit.prevent="createSession" class="max-w-md">
        <div class="mb-4">
          ...
          <input type="text" @input="e => searchMovie((e.target as HTMLInputElement).value)"
            class="w-full px-3 py-2 border rounded">
        </div>

        <div class="mb-4">
          <label class="block text-gray-700 mb-2">Data</label>
          <input v-model="newSession.date" type="date" required class="w-full px-3 py-2 border rounded">
        </div>

        <div class="mb-4">
          <label class="block text-gray-700 mb-2">Hora</label>
          <select v-model="newSession.time" class="w-full px-3 py-2 border rounded">
        ...
        <div class="mb-4">
          <label class="flex items-center">
            <input v-model="newSession.isSpecialDay" type="checkbox" class="mr-2">
            Dia de l'espectador
          </label>
        </div>
